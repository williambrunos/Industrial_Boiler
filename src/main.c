...
void thread_grava_temp_resp(void){
	FILE* dados_f;
	dados_f = fopen("dados_resposta_temperatura.txt", "w");
    if(dados_f == NULL){
...
void thread_grava_tempo_resposta_controlador_nivel(void){
	FILE* dados_f;
	dados_f = fopen("dados_resposta_altura.txt", "w");
    if(dados_f == NULL){
        printf("Erro, nao foi possivel abrir o arquivo\n");
        exit(1);    
    }
...
void thread_grava_temperatura_e_nivel(void){
	char msg_enviada[1000];
	long atraso_fim;
	struct timespec t;
	long periodo = 100e6;
	...
	int amostra = 0, total_amostras = 4000;
	FILE* dados_f;
	dados_f = fopen("dados_temperatura_altura.txt", "w");
	if (dados_f == NULL){
		printf("Erro, nao foi possivel abrir o arquivo\n");
		exit(1);    
	}
	...
	while(amostra < total_amostras){
		clock_nanosleep(CLOCK_MONOTONIC, TIMER_ABSTIME, &t, NULL);
		
		temperatura = sensor_get("t");
		nivel = sensor_get("h");

		fprintf(dados_f, "%lf,%lf\n", temperatura, nivel);
		fflush(dados_f);
	...
	aloca_tela();
	printf("Terminou de gravar no arquivo (temperatura e nível)...\n");
	libera_tela();
...
int main( int argc, char *argv[]) {
	...
	pthread_create(&t8, NULL, (void *) thread_grava_temperatura_e_nivel, NULL);
